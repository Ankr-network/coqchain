...
var (
	_ = coqchain.ChainReader(&Client{})
	_ = coqchain.TransactionReader(&Client{})
	_ = coqchain.ChainStateReader(&Client{})
	_ = coqchain.ChainSyncReader(&Client{})
	_ = coqchain.ContractCaller(&Client{})
	_ = coqchain.GasEstimator(&Client{})
	_ = coqchain.GasPricer(&Client{})
	_ = coqchain.LogFilterer(&Client{})
	_ = coqchain.PendingStateReader(&Client{})
	// _ = coqchain.PendingStateEventer(&Client{})
	_ = coqchain.PendingContractCaller(&Client{})
)
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == coqchain.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != coqchain.NotFound {
		t.Fatal("error should be coqchain.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllPosaProtocolChanges.ChainID) != 0 {
