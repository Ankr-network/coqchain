...
func newTester() *downloadTester {
	...
	tester.stateDb.Put(testGenesis.Root().Bytes(), []byte{0x00}, ethdb.GlobalDataOption)
...
func (dl *downloadTester) CurrentBlock() *types.Block {
	...
	for i := len(dl.ownHashes) - 1; i >= 0; i-- {
		if block := dl.ancientBlocks[dl.ownHashes[i]]; block != nil {
			if _, err := dl.stateDb.Get(block.Root().Bytes(), ethdb.GlobalDataOption); err == nil {
				return block
			}
			return block
		}
...
func (dl *downloadTester) InsertChain(blocks types.Blocks) (i int, err error) {
	...
	for i, block := range blocks {
		...
		} else if _, err := dl.stateDb.Get(parent.Root().Bytes(), ethdb.GlobalDataOption); err != nil {
		...
		dl.stateDb.Put(block.Root().Bytes(), []byte{0x00}, ethdb.GlobalDataOption)
...
func (dlp *downloadTesterPeer) RequestNodeData(hashes []common.Hash) error {
	...
	for _, hash := range hashes {
		if data, err := dlp.dl.peerDb.Get(hash.Bytes(), ethdb.GlobalDataOption); err == nil {
...
func testSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", coqchain.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", coqchain.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", coqchain.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", coqchain.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want coqchain.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", coqchain.SyncProgress{
