...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && (w.chainConfig.Posa == nil || w.chainConfig.Posa.Period > 0) {
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				if w.chainConfig.Posa != nil && w.chainConfig.Posa.Period == 0 {
...
func totalFees(block *types.Block, receipts []*types.Receipt) *big.Float {
	...
	for i, tx := range block.Transactions() {
		...
		minerFee = big.NewInt(0).Add(minerFee, block.BaseFee())
