...
func NewDatabaseWithFreezer(db ethdb.KeyValueStore, freezer string, namespace string, readonly bool) (ethdb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0), ethdb.BlockTxOption); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			...
			if kvhash, _ := db.Get(headerHashKey(frozen), ethdb.BlockTxOption); len(kvhash) == 0 {
			...
			if ReadHeadHeaderHash(db) != common.BytesToHash(kvgenesis) {
				...
				if kvblob, _ := db.Get(headerHashKey(1), ethdb.BlockTxOption); len(kvblob) == 0 {
...
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string, readonly bool) (ethdb.Database, error) {
	kvdb, err := boltdb.NewBoltDB(file)
	if err != nil {
		return nil, err
	}
...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) error {
	it := db.NewIterator(keyPrefix, keyStart, ethdb.GlobalDataOption)
	defer it.Release()
