...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	blocks, _ := GenerateChain(params.AllEthashProtocolChanges, genesis, engine, gendb, basic.chainBlocks, func(i int, b *BlockGen) {})
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	gappedBlocks, _ := GenerateChain(params.AllEthashProtocolChanges, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.gapped, func(i int, b *BlockGen) {})
...
func (snaptest *restartCrashSnapshotTest) test(t *testing.T) {
	...
	newBlocks, _ := GenerateChain(params.AllEthashProtocolChanges, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.newBlocks, func(i int, b *BlockGen) {})
