...
// available in the database. It initialises the default coqchain header
// validator.
...
func (lc *LightChain) SyncCheckpoint(ctx context.Context, checkpoint *params.TrustedCheckpoint) bool {
	...
	if posa := lc.hc.Config().Posa; posa != nil {
		latest -= latest % posa.Epoch // epoch snapshot for clique
	}
	if head >= latest {
		return true
	}
